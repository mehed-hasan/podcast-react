
const supporters=[
    {
        id:1,
        image: require('../components/images/icon/Spotify_logo.png')
    },
    {
        id:2,
        image:require('../components/images/icon/Google Podcast.png')
    },
    {
        id:3,
        image:require('../components/images/icon/Youtube_logo.png')
    }
]


const services = [

    {   id:1,
        name:"Topic by Request",
        details:"Lorem ipsum dolor sit amet consectet pis cing elit, sed do eiusmod tempor.",
        image:"Scribble.png"
    },

    
    {   
        id:2,
        name:"Request by Service",
        details:"Lorem ipsum dolor sit amet consectet pis cing elit, sed do eiusmod tempor.",
        image:"Shining stars.png"
    },

    {   
        id:3,
        name:"One by One",
        details:"Lorem ipsum dolor sit amet consectet pis cing elit, sed do eiusmod tempor.",
        image:"Face 1.png"
    },

    {   
        id:4,
        name:"Request by Process",
        details:"Lorem ipsum dolor sit amet consectet pis cing elit, sed do eiusmod tempor.",
        image:"Coin.png"
    },

    {   
        id:5,
        name:"Static Progression",
        details:"Lorem ipsum dolor sit amet consectet pis cing elit, sed do eiusmod tempor.",
        image:"Fire.png"
    },

    {   
        id:6,
        name:"Important Suggetion",
        details:"Lorem ipsum dolor sit amet consectet pis cing elit, sed do eiusmod tempor.",
        image:"Star.png"
    },


];

const  recentEpisodes= [

    {
        id:1,
        episode_no:1,
        title:"Are you a Perplexed Mind Person?",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac ultrices odio.",
        cardThumbnail:"card_img.png",
        cat:"Women’s Rights",
        cardLink:"#",
        link1:"#",
        link2:"#",
        hostedBy:"John Smith",
        hoster:[
            {   id:1,
                image:"avatar1.png",
                profileLink:"#"
            },
            {   
                id:2,
                image:"avatar2.png",
                profileLink:"#"
            }
        ]
    },

    {
        id:2,
        episode_no:2,
        title:"Are you a Perplexed Mind Person?",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac ultrices odio.",
        cardThumbnail:"card_img2.png",
        cat:"Tesla Autopilot",
        type:'business',
        cardLink:"#",
        link1:"#",
        link2:"#",
        hostedBy:"Abu Bokor",
        hoster:[
            {   id:1,
                image:"avatar1.png",
                profileLink:"#"
            },
            {   id:2,
                image:"avatar2.png",
                profileLink:"#"
            }
        ]
    },


    {
        id:3,
        episode_no:3,
        title:"Are you a Perplexed Mind Person?",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac ultrices odio.",
        cardThumbnail:"card_img3.png",
        type:'business',
        cardLink:"#",
        cat:"Self–confidence",
        link1:"#",
        
        hostedBy:"Ahmad",
        hoster:[
            {   id:1,
                image:"avatar1.png",
                profileLink:"#"
            },
            {   id:2,
                image:"avatar2.png",
                profileLink:"#"
            }
        ]
    },


    {
        id:4,
        episode_no:4,
        title:"Are you a Perplexed Mind Person?",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac ultrices odio.",
        cardThumbnail:"card_img4.png",
        type:'comedy',
        cardLink:"#",
        cat:"Perplexed Mind",
        link1:"#",
        link2:"#",
        hostedBy:"Mehedi Hasan",
        hoster:[
            {   id:1,
                image:"avatar1.png",
                profileLink:"#"
            },
            {   id:2,
                image:"avatar2.png",
                profileLink:"#"
            }
        ]
    },


    {
        id:5,
        episode_no:5,
        title:"Are you a Perplexed Mind Person?",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac ultrices odio.",
        cardThumbnail:"card5.png",
        cat : "Self–confidence",
        type:'education',
        cardLink:"#",
        link1:"#",
        link2:"#",
        hostedBy:"John Law",
        hoster:[
            {   id:3,
                image:"avatar1.png",
                profileLink:"#"
            },
            {   id:4,
                image:"avatar2.png",
                profileLink:"#"
            }
        ]
    },

    {
        id:6,
        episode_no:6,
        title:"Are you a Perplexed Mind Person?",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac ultrices odio.",
        cardThumbnail:"card_img2.png",
        cat:"Inventions",
        type:'health',
        cardLink:"#",
        link1:"#",
        link2:"#",
        hostedBy:"John Smith",
        hoster:[
            {   id:1,
                image:"avatar1.png",
                profileLink:"#"
            },
            {   id:2,
                image:"avatar2.png",
                profileLink:"#"
            }
        ]
    },




];

const sponsorData =[
    {
        id:1,
        price:9.99,
        title: "Single",
        details:"Lorem ipsum dolor sit amet consectet cing elit, sed do eiusmod tempor.",
        features:[
            "Exclusive Content",
            "5% Discount on Merch",
            "Join the Community",
            "Livestreaming Access"
        ],
    },
    
    {
        id:2,
        price:7.99,
        title: "Family",
        details:"Lorem ipsum dolor sit amet consectet cing elit, sed do eiusmod tempor.",
        features:[
            "Everything in Tier 1",
            "Free tickets to Events",
            "Limited Edition Merch",
            "Promote your Product",
            "Request Topic"
        ],
    },


    {
        id:3,
        price:10.05,
        title: "Official",
        details:"Lorem ipsum dolor sit amet consectet cing elit, sed do eiusmod tempor.",
        features:[
            "Everything in Tier 2",
            "Exclusive Badge on",
            "Become an Official Sponsor",
            "Early Access to All Episode",
            "Free Stikers and Merch"
        ],
    },


];

const articles =[

    {
        id:1,
        category:"Podcast",
        title:"Setup your own podcast",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minimvel iam, quis nostrud exercitation ullamco laboris..",
        publishedDate: "Sep 14, 2021",
        type:'business',
        thumbnail:"card2_1.png",
        tags:[
            {   id:1,
                tagName : 'business',
                taglink:'#'
            },
            {   id:2,
                tagName : 'startup',
                taglink:'#'
            }
        ]
    },

    
    {
        id:2,
        category:"Podcast",
        title:"Setup your own podcast",
        details:"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minimvel iam, quis nostrud exercitation ullamco laboris..",
        publishedDate: "Aug 20, 2021",
        type:'comedy',
        thumbnail:"card2_2.png",
        tags:[
            {   id:1,
                tagName : 'business',
                taglink:'#'
            },
            {   id:2,
                tagName : 'Others',
                taglink:'#'
            }
        ]
    },


]


export  {services,recentEpisodes,sponsorData, articles, supporters};
